...
bool Conductor::InitializePeerConnection() {
  ...
  if (!CreatePeerConnection(DTLS_ON)) {
    fprintf(stderr, "CreatePeerConnection failed");
    DeletePeerConnection();
  }
  AddStreams();
  return peer_connection_.get() != NULL;
...
void Conductor::OnAddStream(
  ...
  UIThreadCallback(NEW_STREAM_ADDED, stream.release());
}

void Conductor::OnRemoveStream(
    rtc::scoped_refptr<webrtc::MediaStreamInterface> stream) {
  RTC_LOG(INFO) << __FUNCTION__ << " " << stream->id();
  QueueUIThreadCallback(STREAM_REMOVED, stream.release());
}
...
void Conductor::OnServerConnectionFailure() {
    fprintf(stderr, ("Failed to connect to " + server_).c_str());
}
...
void Conductor::ConnectToPeer(int peer_id) {
  ...
  if (InitializePeerConnection()) {
    ...
    fprintf(stderr, "Failed to initialize PeerConnection");
  }
}
